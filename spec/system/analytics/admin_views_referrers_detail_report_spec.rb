...
describe "Referrers Detail Report" do
  include_context "admin password matches"

  scenario "with no log items" do
    visit "/analytics/referrers/detail/2024/01"
    ...
    expect(description_p.text).to eq "Detail of 0 matching items with referrer (page 1 of 0)."

    headers = page.all("table thead th").map(&:text)
    expect(headers).to eq ["Requested At", "Referrer"]
  ...
  scenario "with a matching log item" do
    ...
    description_p = page.find("main p")
    expect(description_p.text).to eq "Detail of 1 matching item with referrer (page 1 of 1)."

    report_rows = page.all("table tbody tr")
    actual_table_data = report_rows.map(&:text)

    expected_table_data = [
      "01/14/2024 12:00:00am google.com"
    ]
  ...
  scenario "with some matching log items" do
    ...
    description_p = page.find("main p")
    expect(description_p.text).to eq "Detail of 3 matching items with referrer (page 1 of 1)."
    ...
    expected_table_data = [
      "01/14/2024 12:00:00am google.com",
      "01/15/2024 12:00:00am allured.com",
      "01/16/2024 12:00:00am github.com"
    ]
  ...
  scenario "with more than one referrer of log items" do
    FactoryBot.create :apache_log_item, requested_at: Date.new(2024, 1, 16), line_number: 3, referrer_host: "github.com"
    FactoryBot.create :apache_log_item, requested_at: Date.new(2024, 1, 14), line_number: 1, referrer_host: "google.com"
    FactoryBot.create :apache_log_item, requested_at: Date.new(2024, 1, 17), line_number: 4, referrer_host: "jon.zone"
    FactoryBot.create :apache_log_item, requested_at: Date.new(2024, 1, 15), line_number: 2, referrer_host: "allured.com"

    visit "/analytics/referrers/detail/2024/01?page=2"

    description_p = page.find("main p")
    expect(description_p.text).to eq "Detail of 4 matching items with referrer (page 2 of 2)."

    report_rows = page.all("table tbody tr")
    actual_table_data = report_rows.map(&:text)

    expected_table_data = [
      "01/17/2024 12:00:00am jon.zone"
    ]
